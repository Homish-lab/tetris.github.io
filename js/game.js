...
function countPlus (lines0) { //Подсчёт очков
 ...
 document.getElementById('tetriscount').innerHTML = 
  "Линии: "+lines+"<br>Убрано линий: "+count+"<br>Рекорд: "+maxCount;
}
...
function keyPress( key ) { //Обработчик нажатий клавиш
 switch ( key ) {
  case 'escape':    
   window.alert ('Пауза'); //В JS уже есть модальное окно :)
  break;
  case 'left':
   if (valid(-1)) --currentX;
  break;
  ...
  for (var x=0; x<4; x++) {
   if (newCurrent[y][x]) {
    if (typeof(board[y+offsetY])=='undefined' || typeof(board[y+offsetY][x+offsetX])=='undefined'
     ...
     if (offsetY==1) //Конец игры, если текущая фигура - на верхней линии
	 {
                        lose = true; // проиграйте, если текущая фигура установлена в самом верхнем ряду
